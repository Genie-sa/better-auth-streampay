...
- Pending subscription checkouts are only resumed when they bill the same StreamPay consumer — a link created for one payer is never handed to another. Webhook reconciliation enforces the same rule: a provider subscription billed to a different consumer than the local row is dead-lettered (protected correlation) or never matched (fallback).
- Consumer ownership is enforced across models: a consumer linked to an organization can never be claimed by a user, and the other way around (`STREAMPAY_CONSUMER_LINK_CONFLICT`).
- Organization consumer claims are compare-and-set: concurrent first checkouts settle on one consumer, and a consumer created by the losing request is deleted best-effort.
- `organization.getBillingDetails` may supply a billing `email` (organizations have none of their own); `name` and `external_id` stay plugin-owned. New exported types: `BillingOrganization`, `OrganizationBillingOptions`, `OrganizationConsumerOverrides`.
