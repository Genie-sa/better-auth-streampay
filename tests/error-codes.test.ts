...
/**
 * Exhaustive coverage of the raw-code → bucket table plus the status
 * fallback. If anyone retargets a raw code without updating this file,
 * the test locks the new behavior in as a deliberate choice.
 */
...
describe("mapToErrorCode — status fallback", () => {
	...
	it("prefers the raw-code mapping over the status classifier", () => {
		// DUPLICATE_CONSUMER arrives with 409, but our bucket is CONSUMER_DUPLICATE
		// — not whatever 409 would fall through to.
