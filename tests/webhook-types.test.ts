...
describe("webhook type narrowing", () => {
	...
	describe("WebhookHandlers field narrowing", () => {
		...
		it("onInvoiceUpdated pins both discriminants", () => {
			type H = NonNullable<WebhookHandlers["onInvoiceUpdated"]>;
			type Payload = Parameters<H>[0];
			expectTypeOf<Payload["event_type"]>().toEqualTypeOf<"INVOICE_UPDATED">();
			expectTypeOf<Payload["entity_type"]>().toEqualTypeOf<"INVOICE">();
		});

		it("onPaymentLinkPayAttemptFailed pins both discriminants", () => {
			type H = NonNullable<WebhookHandlers["onPaymentLinkPayAttemptFailed"]>;
			type Payload = Parameters<H>[0];
			expectTypeOf<Payload["event_type"]>().toEqualTypeOf<"PAYMENT_LINK_PAY_ATTEMPT_FAILED">();
			expectTypeOf<Payload["entity_type"]>().toEqualTypeOf<"PAYMENT_LINK">();
		});
	});
	...
	// `it.todo` placeholder so the describe block registers even if
	// every assertion is compile-time only — otherwise vitest reports
	// "no tests found" for a purely structural file.
	it("compiles", () => {
		// No runtime work; the assertions above are the test.
	});
});
