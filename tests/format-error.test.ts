...
describe("formatStreamPayError", () => {
	...
	describe("FastAPI validation envelope", () => {
		...
		it("handles a detail entry missing `loc` gracefully", () => {
			const err = mockApiError(422, {
				detail: [{ msg: "something is wrong" }],
			});

			expect(formatStreamPayError(err)).toBe("?: something is wrong");
		});

		it("handles a detail entry missing `msg` gracefully", () => {
			const err = mockApiError(422, {
				detail: [{ loc: ["body", "phone_number"] }],
			});

			expect(formatStreamPayError(err)).toBe("body.phone_number: invalid");
		});
