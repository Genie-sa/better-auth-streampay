...
describe("webhooks plugin", () => {
	...
	describe("plugin creation", () => {
		...
		it("accepts every documented event handler on the options bag", () => {
			const plugin = webhooks({
				secret: SECRET,
				onPayload: vi.fn(),
				onPaymentSucceeded: vi.fn(),
				onPaymentFailed: vi.fn(),
				onPaymentCanceled: vi.fn(),
				onPaymentRefunded: vi.fn(),
				onPaymentMarkedAsPaid: vi.fn(),
				onInvoiceCreated: vi.fn(),
				onInvoiceSent: vi.fn(),
				onInvoiceAccepted: vi.fn(),
				onInvoiceRejected: vi.fn(),
				onInvoiceCompleted: vi.fn(),
				onInvoiceCanceled: vi.fn(),
				onInvoiceUpdated: vi.fn(),
				onSubscriptionCreated: vi.fn(),
				onSubscriptionActivated: vi.fn(),
				onSubscriptionInactivated: vi.fn(),
				onSubscriptionCanceled: vi.fn(),
				onSubscriptionFrozen: vi.fn(),
				onSubscriptionCycleRenewalFailed: vi.fn(),
				onSubscriptionCancelAtPeriodEnd: vi.fn(),
				onSubscriptionFreezeNow: vi.fn(),
				onSubscriptionUnfreezeNow: vi.fn(),
				onSubscriptionUnfreezeFuture: vi.fn(),
	...
	describe("webhook endpoint handler", () => {
		...
		it("rejects an expired signature outside the tolerance window", async () => {
			const body = JSON.stringify({ event_type: "PAYMENT_SUCCEEDED" });
			const ancient = Math.floor(Date.now() / 1000) - 3600;

			const ctx = createMockContext({
				request: makeWebhookRequest(body, signBody(body, ancient)),
			});

			await expect(handler(ctx)).rejects.toThrow(/EXPIRED/);
		});
		...
		it("rejects a malformed signature header", async () => {
			...
			const ctx = createMockContext({
				request: makeWebhookRequest(body, "not-a-real-signature"),
			});

			await expect(handler(ctx)).rejects.toThrow(/MALFORMED_HEADER/);
		});
